    ...
    <>
      <div className='min-h-full'>
        <Disclosure as='nav' className='bg-gray-800'>
          {({ open }) => (
            <>
              <WebMenu open={open} currentPath={currentNav.path} />
              <MobileMenu currentPath={currentNav.path} />
            </>
          )}
        </Disclosure>
...
function WebMenu({ open, currentPath }) {
  return (
              ...
              <Transition as={Fragment} enter='transition ease-out duration-100' enterFrom='transform opacity-0 scale-95' enterTo='transform opacity-100 scale-100' leave='transition ease-in duration-75' leaveFrom='transform opacity-100 scale-100' leaveTo='transform opacity-0 scale-95'>
                <Menu.Items className='absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
                  {userNavigation.map(({ name, href }) => (
                    <Menu.Item key={name}>
                      {({ active }) => (
                        <Link to={href} className={classNames(active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700')}>
                          {name}
                        </Link>
                      )}
                    </Menu.Item>
      ...
      <div className='space-y-1 px-2 pb-3 pt-2 sm:px-3'>
        {navigation?.map(({ name, path }) => (
          <Link key={name} to={path}>
            <Disclosure.Button className={classNames(path === currentPath ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium')} aria-current={path === currentPath ? 'page' : undefined}>
              {name}
            </Disclosure.Button>
          </Link>
        ))}
        ...
        <div className='mt-3 space-y-1 px-2'>
          {userNavigation?.map(({ name, href }) => (
            <Link key={name} to={href}>
              <Disclosure.Button className='block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white'>{name}</Disclosure.Button>
            </Link>
          ))}
        </div>
      </div>
